...
export class VoiceLine {
    ...
    type: 1 | 2 | 3 | null = null
    timesRun = 0;
    limit = 1;
    constructor(character: string, filename: string, type: 1 | 2 | 3, limit?: number) {
        this.type = type;
...
export class VoiceLines {
    ...
    addLine(character: string, filename: string, type: 1 | 2 | 3) {
     ...
     * 
     */
    getLineOfType(type: number) {
        let filteredLines = this.lines.filter((line) => line.type === type);
        let minTimesRun = filteredLines.reduce((prev: number, current)=> current.timesRun < prev ? current.timesRun : prev, 100)
