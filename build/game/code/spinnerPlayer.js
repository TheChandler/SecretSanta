...
export class SpinnerPlayer {
    constructor(speed) {
        this.position = shapeFactory.createVector2(500, 500);
        this.radius = 150; //distance from center to balls center
        this.angle = 0;
        this.ball1 = shapeFactory.createCircle(0, 0, 15);
        this.ball2 = shapeFactory.createCircle(0, 0, 15);
        this.dead = false;
        this.isFrozen = false;
    ...
    positionBalls() {
        this.ball1.position =
            shapeFactory.createVector2(this.position.x + Math.cos(this.angle) * this.radius, this.position.y + Math.sin(this.angle) * this.radius);
        this.ball2.position =
            shapeFactory.createVector2(this.position.x + Math.cos(this.angle + Math.PI) * this.radius, this.position.y + Math.sin(this.angle + Math.PI) * this.radius);
    }
    reset() {
        this.dead = false;
        this.position.x = 500;
